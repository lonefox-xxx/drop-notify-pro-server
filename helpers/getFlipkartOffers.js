...
async function GetFlipkartOffers() {

    try {
        const prevUpdatedOffers = JSON.parse(await client.get("updated_FlipkartOffers")) || [];
...
module.exports = GetFlipkartOffers;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let index = 0; index < hotdeals.length; index++) {
                ...
                if (store && store == 'flipkart' && productUri.Uri) {
...
async function ExtractOfferURL(dataCode) {
    try {
        ...
        console.log(error)
        console.log(`https://www.dealsmagnet.com/buy?${dataCode}`)
